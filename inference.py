"""
Baseline inference script for Pandemic Response Environment.
...
def get_agent_action(client: OpenAI, obs: Dict, step: int, last_reward: float, history: List[str]) -> Dict:
    ...
    try:
        ...
        # Fallback: smart heuristic if model fails
        return _heuristic_action(obs)


def _heuristic_action(obs: Dict) -> Dict:
    """Simple heuristic fallback: vaccinate the most infected city."""
    cities = obs.get("cities", [])
    if not cities:
...
def run_task(client: OpenAI, task_id: str) -> Dict:
    ...
    try:
        ...
        if not done and rewards:
            # Episode ended by step limit — grade current state
