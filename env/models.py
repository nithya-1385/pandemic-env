...
class PandemicAction(BaseModel):
    action_type: str = Field(
        description="One of: allocate_vaccines, set_lockdown, send_resources, no_op"
    )
    city_index: Optional[int] = Field(
        default=None,
        description="0-based index of target city"
    )
    doses: Optional[int] = Field(
        default=None,
        description="Number of vaccine doses to allocate (for allocate_vaccines)"
    )
    enabled: Optional[bool] = Field(
        default=None,
        description="True=lockdown on, False=lockdown off (for set_lockdown)"
    )
    amount: Optional[float] = Field(
        default=None,
        description="Resource units to send (for send_resources)"
    )
    reasoning: Optional[str] = Field(
        default=None,
        description="Optional chain-of-thought from agent"
    )
