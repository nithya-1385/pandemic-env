...
class PandemicSimulation:
    ...
    def compute_reward(self, prev_snapshot: dict, action_feedback: dict) -> float:
        ...
        reward += infection_delta * 10.0   # reward containment

        total_deaths_today = sum(
            c.deaths - prev_snapshot["states"][i]["deaths"]
            for i, c in enumerate(self.states)
        )
        reward -= total_deaths_today * 0.0001  # penalize deaths (scaled)

        # Reward vaccine coverage
        total_vax = sum(c.vaccinated for c in self.states) / len(self.states)
        reward += total_vax * 0.5
